...
def search(request):
    ...
    if request.GET != ' ':
        ...
        if 'category' in request.GET:
            category = request.GET['category']  # request.GET fetches the 'value' of the html object of 'name' = 'category'
            if category:
                queryset_list = queryset_list.filter(category__iexact=category)  # '__iexact' searches in our field for an exact match, again the 'i' in 'iexact' makes it not case sensitive
        ...
        messages.info(request, "We had some issues running your search. Try again later")
        return redirect('/')
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect('core:confirmation', pk = order.id)
